...
void reset_directions(maze_state_t *state)
{
    for (int i = 0; i < DIR; ++i) {
...
int get_valid_direction(maze_state_t *state)
{
    int dir;
...
int try_create_path(char array[][SIZE], maze_state_t *state, int direction)
{
...
void backtrack(maze_state_t *state)
{
    if (state->stack_index > 0) {
        pop_stack(state);
...
bool direction_parse(maze_state_t *state, char array[][SIZE])
{
